...
#bs_params = {
#    'lam': 2048, # The strength of the smoothness parameter
#    'A_diag_min': 1e-3, # Clamp the diagonal of the A diagonal in the Jacobi preconditioner.
#    'cg_tol': 1e-8, # The tolerance on the convergence in PCG
#    'cg_maxiter': 15 # The number of PCG iterations
#}
...
class BSWrapper:
    def __init__(self, imsize=(854, 480), thres=0.60):
        self.imsize = tuple(imsize)
        self.thres = thres

    def build_p(self, ref_p, target_p):
        reference = imread(ref_p)       # HxWxC format
        target = cv2.resize(np.array(Image.open(target_p)), self.imsize, interpolation=cv2.INTER_LINEAR)
        target = (target / np.max(target)).astype('float')
        confidence = np.ones(reference.shape[:2]) * (2^16-1)
        ...
        output_mask = np.zeros(output_solver.shape) 
        output_mask[(output_solver / np.max(output_solver)) > self.thres] = 255

        return output_mask

    def solve(self, ref, tar):
        reference = ref
        target = cv2.resize(tar, self.imsize, interpolation=cv2.INTER_LINEAR)
        target = (target / np.max(target)).astype('float')
        confidence = np.ones(reference.shape[:2]) * (2^16-1)
        im_shape = reference.shape[:2]
...
if __name__ == "__main__":
    ...
    ref_root = "/data/qiang.zhou/media/DAVIS/trainval/JPEGImages/480p"
    vdnames = readvdnames("/data/qiang.zhou/media/DAVIS/trainval/ImageSets/2016/val.txt")
    bsw = BSWrapper()
    for i_vd in vdnames:
        print ("Current on video {}".format(i_vd))
        vd_ref = sorted(glob.glob(os.path.join(ref_root, i_vd, "*.jpg")))[1:]
        vd_tar = sorted(glob.glob(os.path.join(".", i_vd, "*.png")))
        assert(len(vd_ref) == len(vd_tar))
        vd_len = len(vd_ref)
        post_vd_dir = "../post_output_wo_update/{}".format(i_vd)
        os.makedirs(post_vd_dir, exist_ok=True)
        for i in range(vd_len):
            print ("Writing to {}".format(os.path.join(post_vd_dir, vd_tar[i].split('/')[-1])))
            post_im = bsw.build(vd_ref[i], vd_tar[i])
            cv2.imwrite(os.path.join(post_vd_dir, vd_tar[i].split('/')[-1]), post_im)
...
if __name__ == "___main__":
    ...
    reference = imread(os.path.join(data_folder, '00000.jpg'))
    # The 1D image whose values we would like to filter
    target = cv2.resize(np.array(Image.open("./ref_mask.jpg")), (854, 480), interpolation=cv2.INTER_LINEAR)
    cv2.imwrite('upsample.jpg', target)
    target = (target / np.max(target)).astype('float')
    #target = imread(os.path.join(data_folder, 'ref_mask.jpg'))
    ...
    output_mask = np.zeros(output_solver.shape) 
    output_mask[(output_solver / np.max(output_solver)) > 0.65] = 1

    cv2.imwrite('solver.png', output_mask * 255)
...
if __name__ == "_main__":
    ...
    output_filter = (tc_filt / c_filt).reshape(im_shape)
    output_solver = BilateralSolver(grid, bs_params).solve(t, c).reshape(im_shape)
    cv2.imwrite('solver.png', output_solver)

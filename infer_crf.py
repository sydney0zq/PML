...
from net.utils import crf_processing
...
class Postor:
    ...
    def process(self, cur_image_embedding, cur_image, i):
        ...
        output_mask_final = crf_processing(cur_image, output_mask_upsample)
...
def retrieve(dataobj, vdidx, model, output_dir):
    ...
    while pointer <= len(vdidx)-1:
        ...
        for fid, i in enumerate(vdidx[pointer:pointer+infer_bs]):
            ...
            param_pool.append([cur_image_embedding.copy(), cur_data['image'].numpy().transpose((1, 2, 0)).copy()+mean_value, pointer+fid])
...
def main():
    ...
    #parser.add_argument('--bsthres', type=float, default=0.4)
